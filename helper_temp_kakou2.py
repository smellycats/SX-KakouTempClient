...
class TempKakou(object):
    ...
    def get_kakou_info(self, start_id, end_id):
        """根据id范围获取车辆信息"""
        url = 'http://%s:%s/tempkk/final/%s?q={"startid":%s,"endid":%s}' % (
    ...
    def get_maxid(self):
        """根据id范围获取车辆信息"""
        url = 'http://{0}:{1}/tempkk/maxid/{2}'.format(
    ...
    def post_final(self, data):
        url = 'http://{0}:{1}/tempkk/final/{2}'.format(self.host, self.port, self.city)
        try:
    ...
    def post_temp(self, data):
        url = 'http://{0}:{1}/tempkk/temp/{2}'.format(self.host, self.port, self.city)
        try:
            r = requests.post(url, headers=self.headers, data=json.dumps(data))
            if r.status_code == 201:
                return json.loads(r.text)
